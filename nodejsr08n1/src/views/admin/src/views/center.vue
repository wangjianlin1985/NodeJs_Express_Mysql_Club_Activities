  ...
  <div>
    ...
    >  
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='yonghu'"  label="院系" prop="yuanxi">
          <el-select v-model="ruleForm.yuanxi"  placeholder="请选择院系">
            <el-option
                v-for="(item,index) in yonghuyuanxiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='yonghu'"  label="专业" prop="zhuanye">
          <el-select v-model="ruleForm.zhuanye"  placeholder="请选择专业">
            <el-option
                v-for="(item,index) in yonghuzhuanyeOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='yonghu'"  label="班级" prop="banji">
          <el-select v-model="ruleForm.banji"  placeholder="请选择班级">
            <el-option
                v-for="(item,index) in yonghubanjiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='shetuan'"  label="账号" prop="zhanghao">
          <el-input v-model="ruleForm.zhanghao" readonly              placeholder="账号" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='shetuan'"  label="社长姓名" prop="shezhangxingming">
          <el-input v-model="ruleForm.shezhangxingming"               placeholder="社长姓名" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item v-if="flag=='shetuan'"  label="性别" prop="xingbie">
          <el-select v-model="ruleForm.xingbie"  placeholder="请选择性别">
            <el-option
                v-for="(item,index) in shetuanxingbieOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        <el-form-item v-if="flag=='shetuan'" label="头像" prop="touxiang">
          <file-upload
          tip="点击上传头像"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.touxiang?ruleForm.touxiang:''"
          @change="shetuantouxiangUploadChange"
          ></file-upload>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='shetuan'"  label="手机" prop="shouji">
          <el-input v-model="ruleForm.shouji"               placeholder="手机" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='shetuan'"  label="邮箱" prop="youxiang">
          <el-input v-model="ruleForm.youxiang"               placeholder="邮箱" clearable></el-input>
        </el-form-item>
      </el-col>
...
export default {
  data() {
    return {
      ...
      yonghuyuanxiOptions: [],
      yonghuzhuanyeOptions: [],
      yonghubanjiOptions: [],
      shetuanxingbieOptions: [],
    };
  },
  mounted() {
    ...
    this.$http({
      url: `option/yuanxixinxi/yuanximingcheng`,
      method: "get"
    }).then(({ data }) => {
      if (data && data.code === 0) {
        this.yonghuyuanxiOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
    this.$http({
      ...
      if (data && data.code === 0) {
        this.yonghuzhuanyeOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
    this.$http({
      ...
      if (data && data.code === 0) {
        this.yonghubanjiOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
    this.shetuanxingbieOptions = "男,女".split(',')
  },
  methods: {
    yonghutouxiangUploadChange(fileUrls) {
        this.ruleForm.touxiang = fileUrls;
    },
    shetuantouxiangUploadChange(fileUrls) {
        this.ruleForm.touxiang = fileUrls;
    },
    onUpdateHandler() {
      ...
      if((!this.ruleForm.zhanghao)&& 'shetuan'==this.flag){
      ...
      if( 'shetuan' ==this.flag && this.ruleForm.shouji&&(!isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if( 'shetuan' ==this.flag && this.ruleForm.youxiang&&(!isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮箱格式`);
        return
      }
