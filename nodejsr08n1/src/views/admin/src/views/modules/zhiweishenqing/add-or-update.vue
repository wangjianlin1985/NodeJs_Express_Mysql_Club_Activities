<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="社长姓名" prop="shezhangxingming">
          <el-input v-model="ruleForm.shezhangxingming" 
              placeholder="社长姓名" clearable  :readonly="ro.shezhangxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="社长姓名" prop="shezhangxingming">
              <el-input v-model="ruleForm.shezhangxingming" 
                placeholder="社长姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="申请职位" prop="shenqingzhiwei">
          <el-select :disabled="ro.shenqingzhiwei" v-model="ruleForm.shenqingzhiwei" placeholder="请选择申请职位">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="申请职位" prop="shenqingzhiwei">
	      <el-input v-model="ruleForm.shenqingzhiwei"
                placeholder="申请职位" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	shhf : false,
      ...
      rules: {
          ...
          shenqingzhiwei: [
                { required: true, message: '申请职位不能为空', trigger: 'blur' },
          ],
          shenqingliyou: [
          ],
          shenqingshijian: [
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhanghao = true;
          ...
          if(o=='shezhangxingming'){
            this.ruleForm.shezhangxingming = obj[o];
	    this.ro.shezhangxingming = true;
            continue;
          }
	    ...
	    this.ro.shetuanmingcheng = true;
          ...
          if(o=='shenqingzhiwei'){
            this.ruleForm.shenqingzhiwei = obj[o];
	    this.ro.shenqingzhiwei = true;
            continue;
          }
          if(o=='shenqingliyou'){
    ...
    zhanghaoChange () {
      this.$http({
        url: `follow/shetuan/zhanghao?columnValue=`+ this.ruleForm.zhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shezhangxingming){
                this.ruleForm.shezhangxingming = data.data.shezhangxingming
              }
        } else {
          this.$message.error(data.msg);
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhiweishenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhiweishenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
