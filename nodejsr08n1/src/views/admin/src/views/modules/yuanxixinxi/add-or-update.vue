<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="院系名称" prop="yuanximingcheng">
          <el-input v-model="ruleForm.yuanximingcheng" 
              placeholder="院系名称" clearable  :readonly="ro.yuanximingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="院系名称" prop="yuanximingcheng">
              <el-input v-model="ruleForm.yuanximingcheng" 
                placeholder="院系名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='yuanximingcheng'){
            this.ruleForm.yuanximingcheng = obj[o];
	    this.ro.yuanximingcheng = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yuanxixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yuanxixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
